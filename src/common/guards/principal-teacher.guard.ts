...
export class PrincipalOrTeacherGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!authHeader && !cookieHeader) {
            this.logger.warn('No authentication credentials found in request');
        ...
        try {
            ...
            if (user.role !== 'PRINCIPAL' && user.role !== 'TEACHER') {
                this.logger.warn(`Access denied for user ${user.id}: Role is ${user.role}, expected PRINCIPAL or TEACHER`);
